...
int get_cell_value(int h, int w, const int height, const int width, int cell[height][width]) {
    if (h < 0 || height <= h) return 0;
    if (w < 0 || width <= w) return 0;
    return cell[h][w];
}

int my_count_adjacent_cells(int h, int w, const int height, const int width, int cell[height][width]) {
    int count = 0;
    if (get_cell_value(h-1, w-1, height, width, cell)) ++count;
    if (get_cell_value(h-1, w+1, height, width, cell)) ++count;
    if (get_cell_value(h-1, w, height, width, cell)) ++count;
    if (get_cell_value(h, w-1, height, width, cell)) ++count;
    if (get_cell_value(h, w+1, height, width, cell)) ++count;
    if (get_cell_value(h+1, w-1, height, width, cell)) ++count;
    if (get_cell_value(h+1, w, height, width, cell)) ++count;
    if (get_cell_value(h+1, w+1, height, width, cell)) ++count;
...
void my_update_cells(const int height, const int width, int cell[height][width]){
    ...
    for (int  h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
            ...
            int val = cell[h][w];
            if (val) {
                if (num_adjacent != 2 && num_adjacent != 3) val = 0;
            } else {
                if (num_adjacent == 3) val= 1;
            }
